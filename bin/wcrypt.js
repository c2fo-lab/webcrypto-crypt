...
const fs = require('fs')
const os = require('os')
const homePassFile = os.homedir() + '/.wcryptpass'
const readlineSync = require('readline-sync')
const util = require('util')
const readFile = util.promisify(fs.readFile)
const statFile = util.promisify(fs.stat)
const Wcrypt = require('../index.js')
const wcryptStream = require('../lib/node-streams.js')
...
if (!process.stdin.isTTY) {
  ...
  if (yargs.argv.debug) { Wcrypt.DEBUG = true }
  ...
  if (yargs.argv.outfile) {
    if (yargs.argv.debug) { console.error('Will write to "' + yargs.argv.outfile) }
...
// Data from file or command line
  ...
  if (yargs.argv.outfile) {
    if (yargs.argv.debug) { console.error('Will write to file "' + yargs.argv.outfile + '".') }
    destination = fs.createWriteStream(yargs.argv.outfile)
  }

  var source = process.stdin
  if (yargs.argv.infile) {
    if (yargs.argv.debug) { console.error('Will read from file "' + yargs.argv.infile + '".') }
...
function debug (msg) {
  ...
  msg.unshift('[debug] ')
  msg = msg.join(' ')
  if (Wcrypt.DEBUG) { console.error(msg) }
}
...
function getPassphraseFromPrompt (mode) {
  return new Promise((resolve, reject) => {
    ...
    if (!passphrase) { reject(new Error(Config.err.passBlank)) } else if (mode === 'encrypt') {
      ...
      if (parseInt(stats.mode) === 33152) {
        return readFile(passPath)
          .then(data => {
            ...
            if (!passphrase) { throw new Error(Config.err.passBlank) }
            return passphrase
          })
          .catch(err => {
            if (err.code === 'ENOENT') {
